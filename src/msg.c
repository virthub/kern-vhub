...
int klnk_msg_msgget(key_t key, int msgflg)
{
	...
	klnk_msg_log("key=%d, ret=%d", key, ret);
...
out:
	klnk_msg_log("key=%d, cmd=%d, ret=%d", key, cmd, ret);
...
out:
	klnk_msg_log("key=%d, msgtyp=%ld, msgsz=%ld, msgflg=%d, ret=%d", key, msgtyp, msgsz, msgflg, ret);
	klnk_free(buf, buflen);
	return ret;
}
#endif
